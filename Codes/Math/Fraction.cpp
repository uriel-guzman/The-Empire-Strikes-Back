struct Frac {
  ...
  bool operator==(const Frac& f) const { return num == f.num && den == f.den; }

  bool operator!=(const Frac& f) const { return !(*this == f); }

  friend Frac abs(const Frac& f) { return Frac(abs(f.num), f.den); }
  ...
  double operator()() const { return double(num) / double(den); }
