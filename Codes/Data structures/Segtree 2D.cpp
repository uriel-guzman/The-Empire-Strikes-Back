...
struct Seg2D {
  ...
  Seg2D(int x1, int x2, int y1, int y2)
      : x1(x1), x2(x2), tree(0), left(0), right(0) {
    tree = new Dyn(y1, y2);
    if (x1 == x2) return;
    int m = (x1 + x2) >> 1;
  ...
  void pull(int y, lli v) { tree->update(y, max(v, tree->qmax(y, y))); }
  ...
  lli qmax(int xx1, int xx2, int yy1, int yy2) {
    if (xx2 < x1 || x2 < xx1) return -INF;
    if (xx1 <= x1 && x2 <= xx2) return tree->qmax(yy1, yy2);
